...
- 2026-05-30: Live Site Audit Changeset — all 7 phases complete. (1) Font stack: Plus Jakarta Sans, JetBrains Mono, Material Symbols Outlined, 13px base density, `color-mix()` throughout. (2) Brief-as-home: default view is now `recommend`, `BriefComposer` fully wired. (3) Design language pass — Results/Inspector/Pairings: all h2 headings use `--font-display`, all score bars/rings use `--primary`, `tune` icon replaced by `compare` everywhere, CTAs read "Open in Pairings". (4) Design language pass — Library/Home/nav: same `--primary` consolidation, HomeView stat numbers use `--font-display`. (5) Design language pass — Landing/Onboarding: step accent colors, progress bars, and logo marks all flattened to `--primary`; display numbers use `--font-display`. (6) States system: `LoadingState` and `ErrorState` primitives added to `tm-components.jsx` and exported to `window`; `EmptyState` gains `style` spread prop; Pairing Studio saved panel upgraded from bare `<p>` to `EmptyState`. (7) Keyboard shortcuts: global `keydown` listener in app shell — H/B/P/V/L/A navigate views, `[` toggles rail, `?` opens shortcuts overlay, `⌘/Ctrl+,` jumps to Settings; `ShortcutsModal` component with `kbd` styling via `--font-mono`; keyboard icon button added to top bar for discoverability.
